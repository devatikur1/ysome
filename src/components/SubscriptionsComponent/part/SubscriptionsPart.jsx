...
export default function ChannelCard({ channel, UnSubscribe }) {
  console.log(channel);
  ...
  return (
    <article className="flex items-center justify-between w-full gap-5 p-3 py-5 hover:bg-neutral-900/40 rounded-2xl transition-all duration-200 cursor-pointer">
      {/* Left: Photo */}
      <NavLink
        to={`/${channel?.data?.snippet?.customUrl}`}
        className="flex-shrink-0"
      >
        <img
          src={channel?.data?.snippet?.thumbnails?.high?.url}
          alt={channel?.data?.snippet?.title}
      ...
      >
        ...
        <div className="flex items-center gap-1 text-sm text-subtext mt-[2px]">
          <span className="truncate text-subtext/80">
            {channel?.data?.snippet?.customUrl}
          </span>
          {channel?.data?.statistics?.subscriberCount && (
            <>
              <span>•</span>
              <span className="text-subtext/80 text-xs">
                {millify(
                  Number(channel?.data?.statistics?.subscriberCount + 1)
                )}{" "}
                subscribers
              </span>
            </>
        ...
        <p
          ...
          title={channel?.data?.snippet?.description}
        >
      ...
      <section
        onClick={async () => await UnSubscribe({ cdId: channel?.id })}
